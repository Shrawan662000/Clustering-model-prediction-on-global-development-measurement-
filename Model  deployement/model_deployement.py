...
def user_input_features():
    ...
    CO2_Emissions = st.sidebar.number_input('enter CO2 Emissions value',min_value=0.000 )
    ...
    Energy_Usage = st.sidebar.number_input('enter Energy Usage value',min_value=0.000)
    GDP = st.sidebar.number_input('Enter GDP',min_value=0.000)
    Health_Exp_GDP = st.sidebar.number_input('Enter Health Exp % GDP value')
    Health_Exp_Capita = st.sidebar.number_input('Enter Health Exp/Capita',min_value=0.000)
    ...
    Population_Total = st.sidebar.number_input('enter Population Total',min_value=0.000)
    Population_Urban = st.sidebar.number_input('enter Population Urban value')
    Tourism_Inbound = st.sidebar.number_input('enter Tourism Inbound value',min_value=0.000)
    Tourism_Outbound = st.sidebar.number_input('enter Tourism Outbound value',min_value=0.000)

    data = {'Birth Rate':Birth_Rate, 'Business Tax Rate':Business_Tax_Rate, 'log10_CO2 Emissions':np.log10(CO2_Emissions),
       'Days to Start Business':Days_to_Start_Business, 'Ease of Business':Ease_of_Business, 'log10_Energy Usage':np.log10(Ease_of_Business),
...
loaded_model = load(open('Decisiontree.sav', 'rb'))
